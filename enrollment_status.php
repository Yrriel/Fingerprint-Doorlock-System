...
// Validate the data
if ($status && $message) {
    $query = "SELECT * FROM `tablelistfingerprintenrolled` WHERE `ESP32SerialNumber`='$serialnumber' AND `indexFingerprint`='$indexid'";
    $result = $conn->query($query);
    if($result->num_rows > 0)
    // Insert into database (you need to have a `fingerprint_enrollments` table)
    $query = "UPDATE `tablelistfingerprintenrolled` SET `UID`='SET',`indexFingerprint`='$indexid' WHERE `ESP32SerialNumber`='$serialnumber'";
    //
    if($result->num_rows == 0){
